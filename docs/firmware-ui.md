...
- `HOSTS`, `HOST`, and `SYSTEM`: next host;
...
1. `PROXMOX`: global status, host count, guest count, aggregate CPU/RAM/storage pressure, and the top active alert.
...
7. `DETAIL`: selected VM/LXC detail with CPU cores, RAM, disk, uptime, network and disk IO.
8. `ALERTS`: active warnings and critical issues.
